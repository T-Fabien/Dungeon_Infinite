export default class StatusEffect {
    constructor(type, amount, duration, onApply = null, onEnd = null) {
      ...
      if (typeof amount !== "number") {
        throw new Error("Un montant numérique valide est requis pour StatusEffect.");
      }
    ...
    applyEffect(target) {
      ...
      switch (this.type) {
        ...
        case "Armure":
          target.activeArmor = this.amount; // Appliquer l'armure temporaire
          break;
  
        case "Soin":
          target.heal(this.amount);
          console.log(`${target.name} regagne ${this.amount} points de vie.`);
          break;
    ...
    reduceDuration(target) {
      ...
      if (this.duration <= 0) {
        if (this.type === "Armure") {
          ...
          console.log(`${target.name} perd ${this.amount} points d'armure temporaire.`);
        }
